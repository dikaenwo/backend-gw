...
# =======================
try:
    jenis_cocok = pd.read_csv("product - Jenis Kulit Cocok (2).csv")
    jenis_tidak = pd.read_csv("product - Jenis Kulit Tidak Cocok (2).csv")
    masalah_cocok = pd.read_csv("product - Masalah Kulit Cocok.csv")
    masalah_tidak = pd.read_csv("product - Masalah Kulit Tidak Cocok.csv")
    produk_df = pd.read_csv("product - Produk (5).csv")
...
def get_ingredient_details(ingredient, rules_cocok):
    """Get benefit/manfaat for an ingredient"""
    matched = rules_cocok[rules_cocok["Ingredient"] == ingredient]
    if not matched.empty:
        # Ambil manfaat pertama jika ada
        benefit = matched.iloc[0].get("Manfaat", "Baik untuk kulit")
        return benefit
    ...
    try:
        ...
        for idx, ingr in enumerate(ingredients_list):
            ...
            if not rules_cocok.empty and ingr in rules_cocok["Ingredient"].values:
                benefit = get_ingredient_details(ingr, rules_cocok)
            ...
            if not rules_tidak_cocok.empty and ingr in rules_tidak_cocok["Ingredient"].values:
                benefit = get_ingredient_details(ingr, rules_tidak_cocok)
...
if __name__ == '__main__':
    print("🚀 Starting Skincare Recommendation API v2.1...")
    print("📝 Make sure all CSV files are in the same directory")
    print("✨ New features:")
    print("   - Product detail endpoint")
    print("   - Ingredient analysis (cocok & tidak cocok)")
    print("   - Score calculation per ingredient")
